/* ============================================
   SDTM仕様書 Academy - Main Application
   ============================================ */
...
const App = {
    ...
    init() {
        ...
        if (localStorage.getItem('sdtm-spec-darkmode') === 'true') {
    ...
    loadProgress() {
        try {
            const saved = localStorage.getItem('sdtm-spec-progress');
            if (saved) this.progress = JSON.parse(saved);
            const savedQuiz = localStorage.getItem('sdtm-spec-quiz-results');
    ...
    saveProgress() {
        localStorage.setItem('sdtm-spec-progress', JSON.stringify(this.progress));
        localStorage.setItem('sdtm-spec-quiz-results', JSON.stringify(this.quizResults));
    },
        ...
        let html = `
            <div class="fade-in">
                <div class="dashboard-hero">
                    <h2>SDTM仕様書 Academy へようこそ</h2>
                    <p>SDTM仕様書の作成方法を基礎から実践まで学べるプラットフォームです。</p>
                </div>
    ...
    toggleDarkMode() {
        ...
        if (isDark) {
            ...
            localStorage.setItem('sdtm-spec-darkmode', 'false');
        } else {
            document.documentElement.setAttribute('data-theme', 'dark');
            localStorage.setItem('sdtm-spec-darkmode', 'true');
        }
    },
    ...
    resetProgress() {
        if (confirm('全ての学習進捗をリセットしますか？この操作は元に戻せません。')) {
            this.progress = {};
            this.quizResults = {};
            localStorage.removeItem('sdtm-spec-progress');
            localStorage.removeItem('sdtm-spec-quiz-results');
            this.buildSidebar();
            this.updateGlobalProgress();
            this.showDashboard();
        }
